...
for file in `find "$(dirname $0)"/../vendor/github.com/gardener/gardener/.github -name '*.md'`; do
  cat "$file" |\
    sed 's/operating Gardener/working with this Landscaper component/g' |\
    sed 's/to the Gardener project/for this landscaper component/g' |\
    sed 's/to Gardener/to this extension/g' |\
    sed 's/- Gardener version:/- Gardener version (if relevant):\n- Landscaper Component version:/g' |\
    sed 's/\/priority normal//g' \
